...
const workingDirectoryName = path.dirname(require.resolve('pdfkit')).split(path.sep).filter(part => part !== '(rsc)').join(path.sep);
...
function normalizeFilename(fileName) {
  if (fileName.indexOf(workingDirectoryName) === 0) {
    fileName = fileName.substring(workingDirectoryName.length);
  }
